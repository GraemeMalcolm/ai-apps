...
let wllama = null; // Wllama instance for Phi mode
let wllamaReady = false; // Track if wllama is initialized
let wllamaUsedGpu = false; // Whether the loaded wllama instance is using WebGPU
...
let currentMode = 'basic'; // Track which engine is active: 'phi' or 'basic'
const availableModes = { phi: false, basic: true }; // Track which modes can be used
...
const PHI_MODE_FAILURE_MESSAGE = "I'm sorry, something went wrong while generating a response.\nIf this keeps happening, please try switching to Basic mode.";
let lastWllamaCompletionErrored = false; // Track whether last Phi completion failed with an error
...
async function init() {
    ...
    // fine on CPU; keeping it off the GPU leaves the full VRAM budget for
    // the WASM/WebGPU LLM, which is the main cause of device-lost errors
    ...
    try {
        ...
        try {
            await initWllama();
            currentMode = 'phi';
            availableModes.phi = true;
        } catch (error) {
            console.log('Wllama failed to load, using Basic mode (Wikipedia)');
            currentMode = 'basic';
            availableModes.phi = false;
...
async function loadModel() {
    ...
    try {
        ...
        // (which we pin in init() to leave the GPU free for wllama's
        // WebGPU backend) it just wastes time and RAM, so skip it there.
...
/**
 * Initializes the Wllama language model (Phi-2 Orange v2) for Phi mode.
 * Tries to use WebGPU acceleration when available, falling back to CPU-only
 * if WebGPU init fails.
 * @throws {Error} If Wllama initialization fails in both modes
 */
async function initWllama(progressCallback = null, options = {}) {
    const { isLazyLoad = false } = options;
    ...
    if (!isLazyLoad) {
        console.log("Initializing wllama (Phi-2 Orange v2)...");
        updateModelName('Phi-2');
    ...
    const tryLoad = async (configOverrides) => {
        wllama = new Wllama(CONFIG_PATHS);
        await wllama.loadModelFromHF(modelRepo, { ...baseConfig, ...configOverrides });
    };
    ...
    try {
        if (hasWebGPU) {
            // First attempt: let wllama use WebGPU.
            try {
                await tryLoad({ n_gpu_layers: 99 });
                wllamaUsedGpu = true;
                console.log(`Wllama initialized with WebGPU acceleration (${preferredThreads} thread(s))`);
            } catch (gpuErr) {
                console.warn(`WebGPU wllama init failed (${gpuErr.message}), falling back to CPU-only`);
                wllama = null;
                ...
                wllamaUsedGpu = false;
                console.log(`Wllama initialized in CPU-only mode (${preferredThreads} thread(s))`);
            }
        } else {
            await withWebGpuDisabledForWorkers(async () => {
                await tryLoad({
        ...
        if (preferredThreads > 1) {
            console.warn(`Multi-threaded init failed (${multiErr.message}), retrying single-threaded CPU-only`);
            wllama = null;
            try {
                await withWebGpuDisabledForWorkers(async () => {
                    await tryLoad({
                ...
                wllamaUsedGpu = false;
                console.log("Wllama initialized in CPU-only mode (1 thread fallback)");
            } catch (singleErr) {
                console.error('Failed to initialize wllama:', singleErr);
                if (!isLazyLoad) updateLoadingStatus('smollm', 'error', 'Failed');
                wllamaReady = false;
                throw singleErr;
            }
        } else {
            console.error('Failed to initialize wllama:', multiErr);
            if (!isLazyLoad) updateLoadingStatus('smollm', 'error', 'Failed');
            wllamaReady = false;
            throw multiErr;
        }
...
function showTyping() {
    ...
    // Add patience message in Phi mode (model inference can be slow)
    if (currentMode === 'phi') {
        div.innerHTML = `
            <div class="typing">
                <div class="dot"></div>
            ...
            <p style="font-size: 0.85em; color: #666; margin-top: 8px; font-style: italic;">(Responses may be slow. Thanks for your patience!)</p>
        `;
    } else {
        div.innerHTML = `
            <div class="typing">
...
async function handleSend() {
    ...
    // Both Phi and Basic modes use the same non-streaming UI path: show
    // typing indicator, generate, then animate the response into a bubble.
    showTyping();
    ...
    try {
        ...
        if (summary) {
            ...
            if (currentMode === 'phi' && lastWllamaCompletionErrored) {
                addMessage(PHI_MODE_FAILURE_MESSAGE, "bot");
            } else {
        ...
        if ([0, 1, 2, 3, 4].includes(classIndex)) {
            ...
            if ([0, 1, 2, 3].includes(classIndex)) {
                ...
                try {
                    ...
                    // Step 2: Add "I'm researching details..." with thinking dots (and patience note in Phi mode)
                    let researchingMessage = reply + '<br><br>I\'m researching details...<br><br>' +
                        '<div class="typing"><div class="dot"></div><div class="dot"></div><div class="dot"></div></div>';

                    if (currentMode === 'phi') {
                        researchingMessage += '<p style="font-size: 0.85em; color: #666; margin-top: 8px; font-style: italic;">(Responses may be slow. Thanks for your patience!)</p>';
                    }
                    ...
                    } else if (currentMode === 'phi' && lastWllamaCompletionErrored) {
                        setBubbleContent(bubble, reply + `<br><br>${PHI_MODE_FAILURE_MESSAGE}`);
                    }
...
/**
 * Generates computing-related information using the active engine.
...
async function generateComputingInfo(query) {
    if (currentMode === 'phi' && wllamaReady && wllama) {
...
/**
 * Generates text using Wllama with the Phi-2 Orange v2 model (ChatML prompt format).
 */
async function generateWithWllama(query) {
    try {
        lastWllamaCompletionErrored = false;

        // Phi-2 Orange v2 is fine-tuned with ChatML.
...
function selectMode() {
    ...
    if (selected === 'phi') {
        if (!availableModes.phi || !wllamaReady) {
            addMessage('Phi mode is not available. The model failed to load.', 'bot');
            updateModeSelect();
            return;
        }
        currentMode = 'phi';
        addMessage('Switched to Phi mode (Phi-2)', 'bot');
        updateModeSelect();
    } else {
        currentMode = 'basic';
...
function updateModeSelect() {
    ...
    const phiOption = modeSelect.querySelector('option[value="phi"]');
    const basicOption = modeSelect.querySelector('option[value="basic"]');

    if (phiOption) {
        const phiReady = availableModes.phi && wllamaReady;
        phiOption.disabled = !phiReady;
        phiOption.textContent = phiReady ? '🟢 Phi (Phi-2)' : '⚫ Phi (unavailable)';
    }
    if (basicOption) {
        basicOption.textContent = '⚪ Basic (Wikipedia)';
    }

    const modeLabel = currentMode === 'phi' ? 'Phi (Phi-2)' : 'Basic (Wikipedia)';
...
function showAppDetails() {
    ...
    if (modelNameElement) {
        if (currentMode === 'phi' && wllamaReady && wllama) {
            const accel = wllamaUsedGpu ? 'WebGPU' : 'CPU';
            modelNameElement.textContent = `Phi-2 (${accel} - running locally)`;
        } else if (currentMode === 'basic') {
