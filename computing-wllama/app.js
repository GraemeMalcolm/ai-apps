...
async function initWllama(progressCallback = null) {
    try {
        ...
        updateModelName('Phi 4.1 (AI mode)');
    ...
    if (selected === 'cpu') {
        ...
        if (!wllama) {
            ...
            initWllama((progress) => {
                ...
                currentMode = 'cpu';
                if (loadingBubble) {
                    setBubbleContent(loadingBubble, 'Switched to AI mode (Phi 4.1)');
                }
        ...
        currentMode = 'cpu';
        addMessage('Switched to AI mode (Phi 4.1)', 'bot');
        updateModeSelect();
    } else {
        currentMode = 'basic';
...
function updateModeSelect() {
    ...
    if (cpuOption) {
        ...
        cpuOption.textContent = cpuReady ? '🟢 AI mode (Phi 4.1)' : '⚫ AI mode (unavailable)';
    }
    if (basicOption) {
    ...
    const modeLabel = currentMode === 'cpu' ? 'AI mode (Phi 4.1)' : 'Basic (Wikipedia)';
